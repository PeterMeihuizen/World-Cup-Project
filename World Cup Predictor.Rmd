...
WC_matches <- left_join(WC_matches, ind_win_percent, by = c("Winner", "Loser", "year")) %>%
  ...
  mutate(
    home_win_percent = ifelse(Winner == home_team, as.numeric(win_percentage), (100 - win_percentage)),
    away_win_percent = ifelse(Winner == away_team, as.numeric(win_percentage), (100 - win_percentage))
  )
```
...
```{r}
June_12_rankings <- June_12_rankings %>%
